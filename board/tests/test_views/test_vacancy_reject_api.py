...
def test_vacancy_reject_api(user, authenticated_client):
    vacancy = VacancyFactory(status=Vacancy.Status.PENDING, user=user)
    url = reverse('board:vacancy-reject', kwargs={'vacancy_id': vacancy.id})

    response = authenticated_client.post(url)

    vacancy.refresh_from_db()
    assert vacancy.status == Vacancy.Status.REJECTED
    assert response.status_code == status.HTTP_204_NO_CONTENT

@pytest.mark.django_db
def test_vacancy_reject_if_not_admin(authenticated_not_admin):
...
@pytest.mark.django_db
def test_vacancy_reject_if_status_not_pending(user, authenticated_client):
